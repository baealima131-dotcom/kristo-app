/**
 * Pure iOS Church Subscription slot-status resolution (five monthly products).
...
/**
 * Resolve one of the pastor-visible statuses for a monthly slot.
 * Ownership mapping wins over StoreKit availability so a missing Apple
 * product never looks like it belongs to the current Church ID unless
 * the backend mapping says so — and mapped slots keep their ownership status.
 */
