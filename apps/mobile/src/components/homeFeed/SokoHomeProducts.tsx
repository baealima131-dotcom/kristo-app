...
import { ActivityIndicator, Alert, AppState, Dimensions, Image, Keyboard, Linking, Modal, Pressable, ScrollView, Share, StyleSheet, Text, TextInput, View, type NativeScrollEvent, type NativeSyntheticEvent } from "react-native";
...
import {
  beginForcedCheckoutReauth,
  getBlockedCheckoutToken,
  renewKristoCheckoutSession,
  setCheckoutReconnecting,
  sokoCheckoutJson,
...
function SokoProductPhoto({uri,style,resizeMode="cover",width,height}:{uri?:string;style?:any;resizeMode?:"cover"|"contain";width?:number;height?:number}){
  const src=String(uri||"").trim();
  const [failed,setFailed]=useState(false);
  useEffect(()=>{setFailed(false);},[src]);
  const box={width:width??style?.width??"100%",height:height??style?.height};
  if(!src||failed){
    return (
      <View style={[styles.photoPlaceholder,style,box.width!=null||box.height!=null?box:null]}>
  ...
  return (
    <View style={[{overflow:"hidden",backgroundColor:"#EDF2EE"},box.width!=null||box.height!=null?box:null]}>
      <Image source={{uri:src}} style={[style,box]} resizeMode={resizeMode} onError={()=>setFailed(true)}/>
    </View>
  );
}
...
export default function SokoHomeProducts({focused,onProductsChange}:{focused:boolean;onProductsChange:(products:SokoHomeProduct[])=>void}){
  ...
  useEffect(()=>{
    ...
    const publish=(rows:SokoHomeProduct[],reason:string)=>{
      const digest=sokoProductListVisualDigest(rows);
      if(digest===lastDigestRef.current){
        console.log("SOKO_PRODUCTS_SKIP_IDENTICAL",{
          count:rows.length,
          source:reason
        });
        return;
      ...
      onProductsChangeRef.current(rows);
    };
    ...
    async function hydrateCache(){
      try{
        ...
        publish(normalize(parsed),"cache");
      }catch{
...
export const SokoHomeProductCard=React.memo(function SokoHomeProductCard({product,height,layoutWidth}:{product:SokoHomeProduct;height?:number;layoutWidth?:number}){
  const router=useRouter();const {setSession}=useKristoSession();const {initPaymentSheet,presentPaymentSheet}=useStripe();const width=Number(layoutWidth)>0?Number(layoutWidth):Dimensions.get("window").width;const insets=useSafeAreaInsets();const cardWidth=sokoHomeFeedCardWidth(width);const imageHeight=sokoHomeFeedImageHeight(width);const bodyHeight=sokoHomeFeedBodyHeight(width,product);const rowHeight=height??estimateSokoHomeFeedCardHeight(width,product);const detailHeroHeight=Math.min(390,Math.max(320,Math.round(width*0.86)));
  const productKey=sokoHomeFeedProductKey(product);
  console.log("SOKO_CARD_RENDER",{
    productKey,
    reason:"feed-row",
    width:cardWidth,
    imageHeight,
  ...
  const [galleryPage,setGalleryPage]=useState(()=>readSokoCarouselPage(productKey));
  const galleryScrollRef=useRef<ScrollView>(null);
  const loggedLayoutRef=useRef("");
  ...
  const [kristoCheckoutReauth,setKristoCheckoutReauth]=useState(false);
  const [deliveryFailureKind,setDeliveryFailureKind]=useState<""|"session_expired"|"retryable">("");
  const [reconnectIdentifier,setReconnectIdentifier]=useState("");
  const [reconnectPassword,setReconnectPassword]=useState("");
  const [reconnectBusy,setReconnectBusy]=useState(false);
  const [reconnectFormError,setReconnectFormError]=useState("");
  ...
  const checkoutDraftRef=useRef<CheckoutDraft|null>(null);
  ...
  useEffect(()=>{
    const layoutKey=[cardWidth,imageHeight,bodyHeight,rowHeight].join("x");
    if(loggedLayoutRef.current===layoutKey)return;
    const previous=loggedLayoutRef.current;
    loggedLayoutRef.current=layoutKey;
    console.log("SOKO_CARD_LAYOUT_CHANGE",{
      productKey,
      reason:previous?"geometry-update":"geometry-ready",
      width:cardWidth,
      imageHeight,
      bodyHeight,
      cardHeight:rowHeight
    });
  },[cardWidth,imageHeight,bodyHeight,rowHeight,productKey]);
  ...
  const openSecureCheckout=(preferredMethod?: "stripe_card"|"cash_app")=>{
    ...
    setDeliveryError("");
    setDeliveryNotice("");
    setDeliveryFailureKind("");
    setKristoCheckoutReauth(false);
    sessionExpiredRef.current=false;
    resumeDeliveryRatesOnceRef.current=false;
    setReconnectFormError("");
    setCheckoutPaymentError("");
    setCheckoutPaymentNotice("");
    setCheckoutPaymentPhase("idle");
    setConfirmedTotals(null);
  ...
  useEffect(()=>{
    if(sessionExpiredRef.current||reconnectingRef.current){
      return;
    }
    setDeliveryRates([]);
    setSelectedDelivery(null);
  ...
  const captureCheckoutDraft=():CheckoutDraft=>
    snapshotCheckoutDraft({
      productId:realId,
  ...
  const applyCheckoutDraft=(draft:CheckoutDraft|null)=>{
    if(!draft)return;
    setDelivery({
      ...
      streetAddress:draft.delivery.streetAddress,
      apartment:draft.delivery.apartment,
      postalCode:draft.delivery.postalCode,
      instructions:draft.delivery.instructions
    });
    if(draft.paymentMethod==="cash_app"||draft.paymentMethod==="stripe_card"){
      setCheckoutPaymentMethod(draft.paymentMethod);
    }
  };

  useEffect(()=>{
    reconnectingRef.current=kristoCheckoutReauth===true;
    setCheckoutReconnecting(kristoCheckoutReauth===true);
  },[kristoCheckoutReauth]);

  const loadDeliveryRates=async()=>{
    if(!checkoutRequestAllowedWhileReconnecting(reconnectingRef.current)){
      return;
    }
    if(sessionExpiredRef.current&&!resumeDeliveryRatesOnceRef.current){
      setDeliveryFailureKind("session_expired");
      setDeliveryError(SOKO_SESSION_EXPIRED_MESSAGE);
      setCheckoutOpen(true);
      return;
    }
    const resumingExpired=resumeDeliveryRatesOnceRef.current===true;
    resumeDeliveryRatesOnceRef.current=false;
    ...
    try{
      ...
      const kind=classifyCheckoutDeliveryFailure(error);
      setDeliveryFailureKind(kind);
  ...
  loadDeliveryRatesRef.current=loadDeliveryRates;

  const beginCheckoutKristoReauth=()=>{
    checkoutDraftRef.current=captureCheckoutDraft();
    setReconnectFormError("");
    setReconnectPassword("");
    beginForcedCheckoutReauth();
    setCheckoutReconnecting(true);
  ...
  const cancelCheckoutKristoReauth=()=>{
    ...
    setDeliveryNotice(outcome.message||CHECKOUT_REAUTH_CANCELLED_MESSAGE);
    setDeliveryFailureKind("session_expired");
    setDeliveryError(SOKO_SESSION_EXPIRED_MESSAGE);
    sessionExpiredRef.current=true;
    resumeDeliveryRatesOnceRef.current=false;
    setReconnectPassword("");
    setReconnectFormError("");
    setCheckoutReconnecting(false);
    setKristoCheckoutReauth(false);
    setCheckoutOpen(true);
    logCheckoutAuthEvent("home-checkout","cancelled",{
      restoredDraft:true,
      quarantined:true,
      reason:"cancelled"
    });
  ...
  const submitCheckoutKristoReauth=async()=>{
    ...
    try{
      const draft=checkoutDraftRef.current||captureCheckoutDraft();
      const rejectedToken=getBlockedCheckoutToken();
      const next=await renewKristoCheckoutSession({
        identifier:reconnectIdentifier,
        password:reconnectPassword
      });
      await setSession(next as any);
      const outcome=checkoutAfterSessionRenewed({
        draft,
        blockedToken:rejectedToken,
        nextToken:`${String(next.userId||"").trim()}:${String(next.sessionToken||"").trim()}`
      });
      applyCheckoutDraft(outcome.restoreDraft);
      setDeliveryFailureKind("");
      setDeliveryError("");
      setDeliveryNotice("");
      setReconnectPassword("");
      ...
      setKristoCheckoutReauth(false);
      setCheckoutOpen(true);
      if(!resumeDeliveryRatesOnceRef.current){
        resumeDeliveryRatesOnceRef.current=true;
        await loadDeliveryRatesRef.current();
      }
    }catch(error){
      setReconnectFormError(
        String((error as Error)?.message||error||"Kristo sign-in failed.")
      );
      setCheckoutOpen(true);
    }finally{
...
Ukikubali, malipo yatafunguliwa.`
  ...
  const deliveryRecovery=checkoutDeliveryRecoveryActions(deliveryFailureKind,"home-checkout");

  return <View style={{height:bodyHeight,marginTop:SOKO_FEED_CARD_MARGIN_TOP,marginBottom:SOKO_FEED_CARD_MARGIN_BOTTOM}} onLayout={event=>{
    ...
    if(Math.abs(nextHeight-bodyHeight)<1&&Math.abs(nextWidth-cardWidth)<2)return;
    console.log("SOKO_CARD_LAYOUT_CHANGE",{
      productKey,
      reason:"onLayout",
      width:nextWidth,
    ...
    <Pressable style={[styles.media,{height:imageHeight,width:"100%"}]} onPress={()=>setSelected(true)}><SokoProductPhoto uri={product.image} width={cardWidth} height={imageHeight} style={[styles.cover,{width:cardWidth,height:imageHeight}]}/><View style={styles.badge}><Text style={styles.badgeText}>{product.condition.toUpperCase()}</Text></View></Pressable>
      ...
      <ScrollView style={styles.pdScrollFlex} contentContainerStyle={[styles.pdScroll,{paddingBottom:canBuy?24:16+insets.bottom}]}>
        <View style={[styles.pdGallery,{height:detailHeroHeight}]}>
          <ScrollView
            ...
            contentOffset={{x:readSokoCarouselOffset(productKey),y:0}}
            onScroll={event=>{
              const x=Number(event?.nativeEvent?.contentOffset?.x||0);
              writeSokoCarouselOffset(productKey,x);
            }}
            scrollEventThrottle={32}
            onMomentumScrollEnd={event=>{
              const x=Number(event?.nativeEvent?.contentOffset?.x||0);
              const next=Math.max(0,Math.round(x/Math.max(1,width)));
              writeSokoCarouselOffset(productKey,x);
              setGalleryPage(writeSokoCarouselPage(productKey,next,"user-momentum"));
            }}
          >
            {(galleryUris.length?galleryUris:[""]).map((uri,index)=>
              <SokoProductPhoto key={uri||"missing-"+index} uri={uri} width={width} height={detailHeroHeight} style={[styles.pdHero,{width,height:detailHeroHeight}]}/>
            )}
          </ScrollView>
          ...
          <View style={styles.checkoutSection} accessibilityRole="radiogroup">
            ...
            {!!deliveryError?
              <View style={styles.deliveryInlineError}>
                ...
                {deliveryFailureKind==="session_expired"?
                  <Pressable
                    onPress={beginCheckoutKristoReauth}
                    disabled={kristoCheckoutReauth||reconnectBusy}
                    style={styles.deliveryRetry}
                    accessibilityRole="button"
                    accessibilityLabel="Sign in again to continue checkout"
                  >
                    <Text style={styles.deliveryRetryText}>{deliveryRecovery.primaryAction}</Text>
                  </Pressable>
                :deliveryRecovery.showRetry?
                  <Pressable
  ...
  <Modal
    visible={kristoCheckoutReauth}
    transparent
    animationType="fade"
    onRequestClose={cancelCheckoutKristoReauth}
  >
    <View style={styles.reconnectBackdrop}>
      <View style={styles.reconnectCard}>
        ...
        <TextInput
          ...
          onChangeText={setReconnectIdentifier}
        ...
        />
        {!!reconnectFormError?
          <Text style={styles.deliveryInlineErrorText}>{reconnectFormError}</Text>
        :null}
        <Pressable
          onPress={()=>void submitCheckoutKristoReauth()}
        ...
        >
          {reconnectBusy?
            <ActivityIndicator color="#FFF"/>
          :
            <Text style={styles.deliveryRetryText}>Sign in</Text>
          }
        </Pressable>
        <Pressable
          onPress={cancelCheckoutKristoReauth}
          disabled={reconnectBusy}
          accessibilityRole="button"
          accessibilityLabel="Cancel sign-in"
          style={styles.reconnectCancel}
        >
