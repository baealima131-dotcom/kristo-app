...
import {
  ...
  parseChurchOverviewProfileFields,
...
export default function ChurchOverviewScreen() {
  ...
  const [stats, setStats] = useState<OverviewStats>({
    ...
    offeringBalance: initialOverview?.stats.offeringBalance ?? 0,
  });
  ...
  const serverStatsLoadedRef = useRef(false);
  const [statsSource, setStatsSource] = useState<string>("initial");

  async function applyOverviewStatsFromServer(nextStats: OverviewStats, source: string) {
    serverStatsLoadedRef.current = true;
    const sSig = overviewStatsSignature(nextStats);
    statsSigRef.current = sSig;
    setStatsSource(source);
    setStats(nextStats);
    if (__DEV__) {
      console.log("KRISTO_CHURCH_OVERVIEW_STATS_APPLIED", {
        source,
        ministries: nextStats.ministries,
        ministryMembers: nextStats.ministryMembers,
      });
    }
  }

  useEffect(() => {
    serverStatsLoadedRef.current = false;
    setStatsSource("reset");
  }, [churchId, effectiveAuthUserId, refreshAt]);

  useEffect(() => {
    if (!__DEV__) return;
    console.log("KRISTO_CHURCH_OVERVIEW_RENDER_STATS", {
      ministries: stats.ministries,
      ministryMembers: stats.ministryMembers,
      source: statsSource,
      serverStatsLoaded: serverStatsLoadedRef.current,
    });
  }, [stats.ministries, stats.ministryMembers, statsSource]);

  async function load(opts?: { silent?: boolean; manual?: boolean; refreshStats?: boolean }) {
    ...
    try {
      ...
      if (cached) {
        ...
        if (__DEV__) {
          console.log("KRISTO_CHURCH_OVERVIEW_CACHE_HIT", {
            ...
            statsSkipped: true,
            cachedMinistries: cached.stats?.ministries ?? 0,
          });
        }
      } else {
      ...
      const overviewUrl = invitePreview
        ...
        : `${base}/api/church/overview?_=${Date.now()}`;
      ...
      const prevProfileSig = profileSigRef.current;

      const profileFields = parseChurchOverviewProfileFields(churchId, p, {
        uri: cached?.profile?.avatarUri || legacyProfileCache?.avatarUri || legacyProfileCache?.avatarUrl || "",
        updatedAt: cached?.profile?.avatarUpdatedAt || legacyProfileCache?.avatarUpdatedAt,
      });

      const nextProfile: ChurchProfile = {
        ...profileFields,
        avatarUri: profileFields.avatarUri ? mediaUrl(profileFields.avatarUri) : "",
      };

      profileSigRef.current = overviewProfileSignature(nextProfile);
      setProfile(nextProfile);

      console.log("KRISTO_CHURCH_OVERVIEW_PROFILE_SERVER_APPLIED", {
        churchId,
        userId: effectiveAuthUserId,
        name: nextProfile.name,
        pastorName: nextProfile.pastorName,
        hasAvatar: Boolean(nextProfile.avatarUri),
        avatarUpdatedAt: nextProfile.avatarUpdatedAt ?? null,
        refreshStats,
      });

      if (prevProfileSig && prevProfileSig !== profileSigRef.current) {
        console.log("KRISTO_CHURCH_OVERVIEW_PROFILE_STALE_CACHE_REPLACED", {
          churchId,
          userId: effectiveAuthUserId,
          prevProfileSig,
          nextProfileSig: profileSigRef.current,
        });
      }
      ...
      const nextStats = refreshStats
        ...
        : {
            ...
            offeringBalance: cached?.stats?.offeringBalance ?? stats.offeringBalance,
          };

      if (refreshStats) {
        await applyOverviewStatsFromServer(nextStats, "GET /api/church/overview");
      }

      if (__DEV__) {
        console.log("KRISTO_OVERVIEW_MINISTRY_SCOPE", {
          userId: effectiveAuthUserId,
          resolvedUserId: String(j?.data?.viewer?.userId || effectiveAuthUserId),
          churchId,
          serverRole: String(j?.data?.viewer?.role || effectiveAuthRole || "Member"),
          clientRole: effectiveAuthRole,
          scope: "joined",
          joinedMinistryIds: [],
          count: nextStats.ministries,
          ministryMembersCount: nextStats.ministryMembers,
          source: "client:/api/church/overview",
        });
      }
      ...
      await saveChurchOverviewCache({
        ...
        userId: effectiveAuthUserId,
        profile: {
          id: nextProfile.id || String(p?.id || churchId || ""),
          name: nextProfile.name || "",
          address: nextProfile.address || "",
          phone: nextProfile.phone || "",
          pastorName: nextProfile.pastorName || "",
          avatarUri: nextProfile.avatarUri || "",
          avatarUpdatedAt: nextProfile.avatarUpdatedAt,
        },
        stats: nextStats,
        updatedAt: Date.now(),
      });
      ...
      if (session) {
        await setSession({
          ...session,
          churchName: nextProfile.name || session.churchName || churchId,
          churchAvatarUri: nextProfile.avatarUri || "",
          churchAvatarUrl: nextProfile.avatarUri || "",
        } as any);
      }
    } catch (e: any) {
      ...
      if (invitePreview && msg.toLowerCase().includes("no active church membership")) {
        ...
        setStatsSource("invite-preview-empty");
        setErr(null);
      } else {
        setErr(msg);
      }
    ...
    try {
      ...
      const nextAvatarUpdatedAt =
        mergedAvatar.source === "server"
          ? serverUpdatedAt || (nextAvatar ? Date.now() : undefined)
          : mergedAvatar.source === "local"
            ? cached?.avatarUpdatedAt
            : cached?.avatarUpdatedAt;
  ...
  const applyChurchProfileEvent = useCallback(
    async (payload: { name?: string; avatarUri?: string; avatarUrl?: string; avatarUpdatedAt?: number }) => {
      ...
      const nextUpdatedAt = payload.avatarUpdatedAt || cached?.avatarUpdatedAt || Date.now();
      setProfile((prev) => ({
        ...prev,
      ...
      if (session && nextAvatar) {
        await setSession({
          ...session,
          churchName: String(payload.name || cached?.name || session.churchName || ""),
          churchAvatarUri: nextAvatar,
          churchAvatarUrl: nextAvatar,
        } as any);
      }
  ...
  useEffect(() => {
    ...
    const needsServerStats = !serverStatsLoadedRef.current;
    const refreshStats =
      force ||
      needsServerStats ||
      shouldAllowScreenRefresh("ChurchOverview", {
        forceKey: refreshAt,
        minMs: CHURCH_TAB_REFRESH_MS,
      });

    const run = () =>
      load({
        silent: hasOverviewCacheRef.current || !needsServerStats,
        manual: force || needsServerStats,
        refreshStats,
      });

    if (profileFetchInflightRef.current) {
      void profileFetchInflightRef.current.finally(run);
      return;
    }
    ...
    profileFetchInflightRef.current = job;
  }, [isFocused, refreshAt, invitePreview, previewChecked, previewLimitReached, churchId, effectiveAuthUserId]);

  useEffect(() => {
    if (invitePreview) return;
    if (!churchId || !effectiveAuthUserId) return;
    if (serverStatsLoadedRef.current) return;
    void load({ silent: false, manual: true });
  }, [churchId, effectiveAuthUserId, invitePreview]);
