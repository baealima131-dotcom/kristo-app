...
export async function POST(
  ...
  try {
    ...
    const purchase =
      await dbGetSokoSupplyPurchaseRequest({
        sellerUserId:
          owner.sellerUserId,
    ...
    if (
      ...
      throw new Error(
        "Rejected purchase request cannot be ordered."
      );
    }
    ...
    if (
      ...
      return NextResponse.json({
        ...
        message:
          "This Alibaba listing needs a supplier quote. Continue on Alibaba to confirm final price, shipping and order terms before payment.",
    ...
    const result =
      await placeSokoSupplyOrder({
        provider:
          purchase.provider as
            "alibaba",
