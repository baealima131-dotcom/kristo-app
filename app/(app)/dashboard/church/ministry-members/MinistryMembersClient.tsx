...
import { addMinistryMember, removeMinistryMember, updateMinistryMemberRole } from "@/lib/api/ministryMembers";
...
export default function MinistryMembersPage() {
  ...
  if (!ministryId) {
    setError("Weka Ministry ID kwanza");
    return;
  }
  if (!selectedUserId) {
    setError("Chagua member kwanza");
    return;
  }

  const cm = churchMembers.find((m) => m.userId === selectedUserId);
  if (!cm) {
    setError("Member hapatikani");
    return;
  }
  ...
  try {
    const json = await addMinistryMember({
      ministryId,
      userId: cm.userId,
      role,
    });
  ...
  setError("");
  try {
    const json = await updateMinistryMemberRole({ id: memberId, role: next });
    if (!json.ok) {
  ...
  setError("");
  try {
    const json = await removeMinistryMember(id);
    if (!json.ok) {
