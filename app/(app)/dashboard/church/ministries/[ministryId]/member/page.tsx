...
export default function MinistryMemberPage() {
  ...
  return (
    <div style={wrap}>
      <div style={card}>
        <div style={{ fontSize: 22, fontWeight: 1000 }}>👤 Member Space</div>
        <div style={{ opacity: 0.8, marginTop: 8 }}>
          Ministry ID: <span style={{ fontWeight: 900 }}>{ministryId || "(missing)"}</span>
        </div>

        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 14 }}>
          <Link style={btn} href={`/dashboard/church/ministries/${encodeURIComponent(ministryId)}/member/announcements`}>
            📣 Announcements
          </Link>
          <Link style={btn} href={`/dashboard/church/ministries/${encodeURIComponent(ministryId)}`}>
            ← Back to Ministry Profile
          </Link>
          <Link style={ghost} href="/dashboard/church/notifications">
            🔔 Notifications
          </Link>
        </div>

        <div style={{ opacity: 0.7, marginTop: 14, fontSize: 12 }}>
          Tip: notification deep-link inaweza kuleta ?ann=ANN_ID kwenye announcements list.
        </div>
      </div>
    </div>
  );
