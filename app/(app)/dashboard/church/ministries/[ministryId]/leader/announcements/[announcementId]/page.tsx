...
export default function MemberAnnouncementDetailPage() {
  ...
  const headerAuth = useMemo(() => {
    // DEV: headers-based auth (same pattern you use elsewhere).
    // If you already have a helper to build these headers, replace this block.
    const uid = (localStorage.getItem("KRISTO_DEV_USER_ID") || "").trim();
    const role = (localStorage.getItem("KRISTO_DEV_ROLE") || "").trim();
    const cid = (localStorage.getItem("KRISTO_DEV_CHURCH_ID") || "").trim();
    ...
    <div style={wrap}>
      ...
      {loading ? (
        ...
        <div style={card}>
          ...
          {item.message ? <div style={msg}>{item.message}</div> : <div style={msg}>(No message)</div>}
        </div>
      )}
    </div>
  );
