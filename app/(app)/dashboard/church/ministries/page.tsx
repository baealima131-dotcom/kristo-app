...
const sorted = useMemo(() => {
  ...
  async function load() {
    ...
    setError("");
    try {
      const res = await fetch("/api/church/ministries", {
        cache: "no-store",
        credentials: "include",
        headers: { ...devHeaders(), accept: "application/json" },
      });

      const json = await readApi<Ministry[]>(res);

      if (!res.ok || !okJson(json)) {
        const msg = json && !okJson(json) ? (json as ApiErr).error : "";
        setError(explainAuthProblem(res.status, msg || "Failed to load ministries"));
        setItems([]);
        return;
      }
  ...
  async function create() {
    ...
    try {
      ...
      if (!res.ok || !okJson(json)) {
        const msg = json && !okJson(json) ? (json as ApiErr).error : "";
        setError(explainAuthProblem(res.status, msg || "Failed to create ministry"));
        return;
      }
  ...
  async function update(m: Ministry, patch: Partial<Pick<Ministry, "name" | "description" | "status">>) {
    setLoading(true);
    setError("");
    try {
      const res = await fetch(`/api/church/ministries?id=${encodeURIComponent(m.id)}`, {
        method: "PATCH",
        credentials: "include",
        headers: { ...devHeaders(), "content-type": "application/json", accept: "application/json" },
      ...
      if (!res.ok || !okJson(json)) {
        const msg = json && !okJson(json) ? (json as ApiErr).error : "";
        setError(explainAuthProblem(res.status, msg || "Failed to update ministry"));
        return;
      }
  ...
  async function removeMinistry(id: string) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/church/ministries?id=${encodeURIComponent(id)}`, {
        method: "DELETE",
        credentials: "include",
        headers: { ...devHeaders(), accept: "application/json" },
      });

      const json = await readApi<{ id: string }>(res);

      if (!res.ok || !okJson(json)) {
        const msg = json && !okJson(json) ? (json as ApiErr).error : "";
        setError(explainAuthProblem(res.status, msg || "Failed to delete ministry"));
        return;
      }
  ...
  async function loadMembers(mid: string) {
    ...
    setMembersError(mid, "");
    try {
      const res = await fetch(`/api/church/ministry-members?ministryId=${encodeURIComponent(mid)}`, {
        cache: "no-store",
        credentials: "include",
        headers: { ...devHeaders(), accept: "application/json" },
      });

      const json = await readApi<MinistryMember[]>(res);

      if (!res.ok || !okJson(json)) {
        const msg = json && !okJson(json) ? (json as ApiErr).error : "";
        setMembersError(mid, explainAuthProblem(res.status, msg || "Failed to load members"));
      ...
      setMembersError(mid, "Network error");
    } finally {
  ...
  async function addMember(mid: string) {
    ...
    if (!userId) return alert("Chagua member kwenye dropdown (au hakikisha kuna members kwenye Church Members).");
    ...
    try {
      const res = await fetch("/api/church/ministry-members", {
        ...
        credentials: "include",
        headers: { ...devHeaders(), "content-type": "application/json", accept: "application/json" },
        body: JSON.stringify({ ministryId: mid, userId, role }),
      });
      ...
      if (!res.ok || !okJson(json)) {
        const msg = json && !okJson(json) ? (json as ApiErr).error : "";
        setMembersError(mid, explainAuthProblem(res.status, msg || "Failed to add member"));
        return;
      }

      // reset selection
      setAddUserIdByMinistry((cur) => ({ ...cur, [mid]: "" }));
      setAddRoleByMinistry((cur) => ({ ...cur, [mid]: "Member" }));
  ...
  async function changeMemberRole(mid: string, mm: MinistryMember, nextRole: MinistryMemberRole) {
    ...
    setMembersError(mid, "");
    try {
      const res = await fetch(`/api/church/ministry-members?id=${encodeURIComponent(mm.id)}`, {
        method: "PATCH",
        credentials: "include",
        headers: { ...devHeaders(), "content-type": "application/json", accept: "application/json" },
      ...
      if (!res.ok || !okJson(json)) {
        const msg = json && !okJson(json) ? (json as ApiErr).error : "";
        setMembersError(mid, explainAuthProblem(res.status, msg || "Failed to update role"));
        return;
      }
  ...
  async function removeMember(mid: string, mm: MinistryMember) {
    ...
    try {
      ...
      if (!res.ok || !okJson(json)) {
        const msg = json && !okJson(json) ? (json as ApiErr).error : "";
        setMembersError(mid, explainAuthProblem(res.status, msg || "Failed to remove member"));
        return;
      }
          ...
          <div style={{ display: "grid", gap: 12 }}>
            {sorted.map((m) => {
              ...
              const filteredChurchMembers = !search
                ? churchMembers
                : churchMembers.filter((cm) => {
                    const a = String(cm.name || "").toLowerCase();
                        ...
                        {!mMembers ? (
                          ...
                          <div style={{ display: "grid", gap: 10 }}>
                            ...
                            return mMembers.map((mm) => {
                              ...
                              const disableAssistant = mm.role !== "Assistant" && hasAssistant;
                              const disableLeader = mm.role !== "Leader" && hasLeader;
                                                            return (
                                <div
                                  key={mm.id}
                                  style={{
